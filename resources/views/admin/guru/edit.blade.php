    ...
    <div class="py-12 px-6">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="mb-8 text-center">
                <h2 class="text-3xl font-bold text-gray-800">Edit Akun Guru</h2>
                <p class="text-gray-500 mt-2">Perbarui informasi akun guru melalui formulir di bawah ini.</p>
            </div>
                ...
                <form method="POST" action="{{ route('admin.guru.update', $guru->id) }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="status_kepegawaian" id="status_kepegawaian"
                            ...
                            <option value="honor" {{ old('status_kepegawaian', $guru->status_kepegawaian) == 'honor' ? 'selected' : '' }}>Guru Honor</option>
                            <option value="pns" {{ old('status_kepegawaian', $guru->status_kepegawaian) == 'pns' ? 'selected' : '' }}>Guru PNS</option>
                            <option value="pppk" {{ old('status_kepegawaian', $guru->status_kepegawaian) == 'pppk' ? 'selected' : '' }}>Guru PPPK</option>
                        </select>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">
                            <span id="nip-label">{{ in_array($guru->status_kepegawaian, ['pns', 'pppk']) ? 'NIP' : 'NUPTK' }}</span> 
                            <span class="text-gray-400 font-normal">(Wajib)</span>
                        </label>
                        <input type="text" name="nip" id="nip" value="{{ old('nip', $guru->nip) }}" 
                            class="w-full border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition shadow-sm placeholder-gray-400"
                            placeholder="{{ in_array($guru->status_kepegawaian, ['pns', 'pppk']) ? 'Nomor Induk Pegawai' : 'Nomor Unik Pendidik dan Tenaga Kependidikan' }}">
                        <p class="text-xs text-gray-500 mt-2" id="nip-hint">
                            {{ $guru->status_kepegawaian == 'pns' ? 'NIP untuk guru PNS' : ($guru->status_kepegawaian == 'pppk' ? 'NIP untuk guru PPPK' : 'NUPTK untuk guru honor') }}
                        </p>
                    </div>
